...
class PretrainLearningConfig:
    loss_type = "QLoss"  # TODO: loss function type, chose in ["NSELoss" ,"MSE"]
    loss_functions = {"MSE": nn.MSELoss(), "NSELoss": nseloss.NSELoss(), "QLoss": nseloss.QuantileLoss(0.5)}
    ...
    scheduler_paras = {"scheduler_type": "warm_up", "warm_up_epochs": n_epochs * 0.25, "decay_rate": 0.90
                       }
...
class PretrainConfig(PretrainLearningConfig):
    ...
    used_model = "TFRN"  # TODO # Transformer
    ...
    if DatasetConfig.decompose:
        pre_train_id = f"{DatasetConfig.forcing_type}{DatasetConfig.basin_mark}~{DatasetConfig.decompose}@{DataShapeConfig.data_shape_info}" \
                       f"@{DatasetConfig.train_start.date()}~{DatasetConfig.train_end.date()}"
        pre_val_id = f"{DatasetConfig.forcing_type}{DatasetConfig.basin_mark}~{DatasetConfig.decompose}@{DataShapeConfig.data_shape_info}" \
                     f"@{DatasetConfig.val_start.date()}~{DatasetConfig.val_end.date()}"
        pre_test_id = f"{DatasetConfig.forcing_type}{DatasetConfig.basin_mark}~{DatasetConfig.decompose}@{DataShapeConfig.data_shape_info}" \
    ...
    pre_train_config = {
        ...
        "decompose": DatasetConfig.decompose
    }
    ...
    saving_message = f"{model_info}_{DatasetConfig.dataset_info}_{DataShapeConfig.data_shape_info}" \
                     f"_{PretrainLearningConfig.learning_config_info}_seed{seed}"
