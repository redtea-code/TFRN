...
from data.dataset_aus import CamelsAusDataset2, CamelsAusDatasetWithStatic, CamelsAusDatasetWithStatic_decompose
...
class StaticReader(AbstractStaticReader):
    ...
    def __init__(self, camels_root):
        ...
        self.static_file_path = self.camels_root / "attribute_merge" / "b.csv"
        self.df_static = pd.read_csv(self.static_file_path, header=0, dtype={"gauge_id": str}).set_index("gauge_id")
        self.df_static = self.df_static.select_dtypes("float").astype("float32")
...
class CamelsDatasetWithStatic_decompose(CamelsDataset):
    """CAMELS dataset with static attributes injected into serialized sequences.
    Inherited from NullableCamelsDataset
    """
    ...
    def _load_data(self, forcing_type):
        ...
        for idx, basin in enumerate(self.basins_list):
            ...
            if self.decompose == "mvmd" and self.decompose_var == "y":
                ...
                [u, u_hat, omega] = mvmd(y, 2000, 0, 3, 0, 1, 1e-7, 50)
                y = u.permute(2, 1, 0)[0].numpy()
                y = np.append(y, y_copy, axis=1)
            elif self.decompose == "memd" and self.decompose_var == "y":
                y_copy = y.copy()  # 分解前的y
                imfs = select_imfs_by_variance(y[:, 0], target_num=3)
                y = np.append(imfs.transpose(1, 0), y_copy, axis=1)
                ...
                y_ssa = ssa.fit_transform(y.T)
                y = np.append(y_ssa[0].T, y_copy, axis=1)

            elif self.decompose == "stl" and self.decompose_var == "y":
                y_copy = y.copy()  # 分解前的y
                stl = STL(df_y.iloc[:, 0], period=365, seasonal=9)
                result = stl.fit()
                y = np.vstack([result.seasonal, result.trend, result.resid]).transpose(1, 0)
                y = np.append(y, y_copy, axis=1)
            ...
            # Calculate mean and std in training stage
            if self.stage == 'train':
                self.y_stds_dict[basin] = y.std(axis=0)
...
class DatasetFactory:
    @staticmethod
    def get_dataset_type(use_future_fea, use_static, dataset="US", decompose=False):

        if dataset == "AUS":
            if decompose:
                ds = CamelsAusDatasetWithStatic_decompose
            elif use_static:
                ds = CamelsAusDatasetWithStatic
            else:
                ds = CamelsAusDataset2
            return ds

        if (not use_future_fea) and use_static:
            raise RuntimeError("No implemented yet.")
        elif decompose:
            ds = CamelsDatasetWithStatic_decompose
...
if __name__ == '__main__':
    ...
    # basins_file = r"D:\RR-Former\camels_aus\aus_222basins_list.txt"
    ...
    pred_len = 7
    stage = "train"
    ds = CamelsDatasetWithStatic_decompose(camels_root=camels_root, forcing_type=forcing_type,
                                           basins_list=global_basins_list[:1],
