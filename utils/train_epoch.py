...
def train_epoch_withstatic(model, data_loader, optimizer, scheduler, loss_func, decode_mode, device, k=3):
    """
    Train model for a single epoch.
    ...
    cnt = 0
    loss_mean = 0
    for x_seq, y_seq_past, y_seq_future, y_stds, static_fea in data_loader:
        # delete previously stored gradients from the model
        optimizer.zero_grad()
        # push data to GPU (if available)
        x_seq, y_seq_past, y_seq_future = x_seq.to(device), y_seq_past.to(device), y_seq_future.to(device)
