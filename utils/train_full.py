...
from utils.train_epoch import train_epoch_withstatic
from utils.eval_model import eval_model_withstatic
...
def train_full(model, decode_mode, train_loader, val_loader, optimizer, scheduler, loss_func, n_epochs, device,
    ...
    for i in tqdm(range(n_epochs)):
        train_loss_iterated = train_epoch_withstatic(model, train_loader, optimizer, scheduler, loss_func, decode_mode, device)
        # mse_train, nse_train = '', ''
        # mse_train, nse_train is not need to be calculated (via eval_model function),
        # and you can comment the next line to speed up
        mse_train, nse_train = eval_model_withstatic(model, train_loader, decode_mode, device)
        mse_val, nse_val = eval_model_withstatic(model, val_loader, decode_mode, device)
