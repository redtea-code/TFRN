...
def eval_model_withstatic(model, data_loader, decode_mode, device,k=3):
    """
    Evaluate the model.
    ...
    nse_mean = 0
    with torch.no_grad():
        for x_seq, y_seq_past, y_seq_future, _, static_fea in data_loader:
            x_seq, y_seq_past, y_seq_future = x_seq.to(device), y_seq_past.to(device), y_seq_future.to(device)
            ...
            if decode_mode == "NAR":
                ...
                for i in range(tgt_len - pred_len, tgt_len):
                    ...
                    dec_inputs[:, i, :] = decoder_predict[:, i - 1, :]
                y_hat = dec_inputs[:, -pred_len:, :]
            elif decode_mode == "mvmd":
                y_seq_past, _, _ = mvmd_batch(y_seq_past.permute(0, 2, 1), 2000, 0, k, 0, 1, 1e-7, 50)
                y_hat = model(x_seq, y_seq_past[:, :, 0, :], static_fea)
            elif decode_mode == "mvmd_all":
                y_seq_past, _, _ = mvmd_batch(y_seq_past.permute(0, 2, 1), 2000, 0, k, 0, 1, 1e-7, 50)
                x_seq, _, _ = mvmd_batch(x_seq.permute(0, 2, 1), 2000, 0, k, 0, 1, 1e-7, 50)
                y_hat = model(x_seq.reshape(batch_size, -1, past_len), y_seq_past.reshape(batch_size, -1, past_len),
                              static_fea)
            elif decode_mode == "memd":
                y_seq_past, _, _ = memd_torch(y_seq_past)
                y_hat = model(x_seq, y_seq_past.reshape(batch_size, -1, past_len), static_fea)
            elif decode_mode == "memd_all":
                seq_all = torch.cat([x_seq[:, :past_len, :], y_seq_past], dim=2)
                seq_all, _ = memd_torch(seq_all)
                y_hat = model(seq_all.reshape(batch_size, -1, past_len), static_fea)
            else:  # Model is not Transformer
                y_hat = model(x_seq, y_seq_past, static_fea)
...
def eval_model_obs_preds_withstatic(model, data_loader, decode_mode, device,k=3):
    """
    Evaluate the model.
