...
def calc_nse(obs: np.array, sim: np.array) -> np.array:
    ...
    return nse_mean, nse[:, 0]  # 索引是为了去掉-1维度
...
def calc_tpe(obs: np.array, sim: np.array, alpha=0.02):
...
def calc_mae(obs: np.array, sim: np.array):
    mae = np.mean(np.abs(obs - sim), axis=0)
    mae_mean = np.mean(mae)
    # Return mean MSE, and MSE of all locations, respectively
    return mae_mean, mae[:, 0]

def calc_bias(obs: np.array, sim: np.array):
    """Pbias,百分比偏差，又称相对误差，
    表示测量模拟的水流的百分比偏差大于或小于相应的推断的自然水流，越接近0模型效果越好"""
...
def calc_fhv(obs: np.array, sim: np.array,alpha=0.02):
    # Step 1: Sort the observed and simulated values
    sort_index = np.argsort(obs, axis=0)  # Get the sorted indices based on observed values
    obs_sort = np.take_along_axis(obs, sort_index, axis=0)  # Sort the observed values
    sim_sort = np.take_along_axis(sim, sort_index, axis=0)  # Sort the simulated values
    # Step 2: Determine the top alpha% for high flow values
    top = int(obs.shape[0] * alpha)  # Get the index corresponding to the top alpha%
    obs_t = obs_sort[-top:, :]  # Extract the top alpha% observed high flow values
    sim_t = sim_sort[-top:, :]  # Extract the top alpha% simulated high flow values

    # Step 3: Calculate the percentage deviation
    numerator = np.sum(sim_t - obs_t, axis=0)
    denominator = np.sum(obs_t, axis=0)
    fhv = numerator / denominator

    fhv_mean = np.mean(fhv)

    return fhv_mean, fhv[:, 0]