...
def calculate_return_period_performance_metrics(
    ...
    predictions.index = [
        pd.to_datetime(t, unit='ns', origin='unix') for t in predictions.index]
    observations.index = [
        pd.to_datetime(t, unit='ns', origin='unix') for t in observations.index]
    ...
    for return_period in evaluation_utils.RETURN_PERIODS:
        for window in RETURN_PERIOD_TIME_WINDOWS:
            ...
            # print(metrics, sim_return_period_dict)
